...
namespace WootingCursor
{
    internal static class Program
    {
        static void device_event_cb(DeviceEventType eventType, DeviceInfo deviceInfo)
        {
            ...
            {
                Cursor.Position = new Point(Cursor.Position.X + (int)(WootingAnalogSDK.ReadAnalog(30).Item1 * 100), 
                    Cursor.Position.Y);
            }
        }
        ...
        static void timer_cb(object state)
        {
            ...
            var ret = WootingAnalogSDK.SetKeycodeMode(KeycodeType.VirtualKey);
                ...
                while (true)
                {
                    ...
                    if (readErr == WootingAnalogResult.Ok)
                    {
                        foreach (var analog in read)
                        {
                            ...
                            {
                                freshOutput += $"({(VirtualKeys)analog.Item1},{analog.Item2}) v";
                            }
